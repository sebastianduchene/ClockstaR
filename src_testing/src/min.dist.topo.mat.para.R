...
#		print(paste("step", i-1))
		...
		compute.tree.dists <- function(tree.sub.list, fix.tree){
			res <- sapply(tree.sub.list, function(a){ return(min.dist.topo(fix.tree, a))})
			return(res)
		}

# > foreach(i = list(list(1, 4), list(2), list(3)), j = 1:3) %do% paste(i, nums[[j]])
...
#				topo.dist[[j]] <- c(0, 0, 0)
#			}else if(j != i){
#				tree.list.ev <- tree.list[1:j]
#				topo.dist[[j]] <- sapply(tree.list.ev, function(a){ return(min.dist.topo(tree.list[[i]], a))})
#			}
