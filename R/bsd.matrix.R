...
function(tree.list){
	...
	if(length(tree.list) > 3){
		d.mat.lin <- vector()
			d.mat.lin <- sapply(2:nrow(d.mat), function(a){print(paste("estimating distances",a-1, "of", nrow(d.mat)-1)); lapply(tree.list[1:(a-1)], function(y){bsd.dist(tree1=y, tree2=tree.list[[a]]) })  })
